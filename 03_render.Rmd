...
To validate the use of averaged skill cluster scores, we compared their distribution to the original O\*NET importance ratings for individual skills, as mapped to Canadian occupations. Because cluster scores are derived by averaging multiple individual skills, their distribution is smoother and exhibits less variability, particularly at the extremes(Figure \@ref(fig:fig8)). Applying fixed importance thresholds from the raw O\*NET scale (e.g., scores of 25 and 50) directly to the cluster scores would risk understating importance due to this smoothing effect. To address this, we realigned the thresholds based on percentile positions in the original distribution:
...
* We then located the corresponding 30th and 60th percentiles in the skill cluster distribution, which yielded new threshold values (e.g., ~22.3 and ~38.6).
* These recalibrated values were used to categorize cluster-level scores into importance tiers, preserving the relative interpretation of skill importance.
